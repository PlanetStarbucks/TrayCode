...
const eraserFormat = function (wfAry) {
  ...
  const icons = JSON.parse(
    fs.readFileSync(
      "/Users/bill.keiffer/Desktop/Git/TrayCode/EraserExport/IconLookup/trayConnectors.json"
    )
  );
  const colors = JSON.parse(
    fs.readFileSync(
      `/Users/bill.keiffer/Desktop/Git/TrayCode/EraserExport/IconLookup/colorArray.json`
    )
  );
    ...
    if (stepAry.length === 0) {
      ...
      for (let i = 0; i < stepAry.length; i++) {
        if (stepAry[i].type === "normal" || stepAry[i].type === "break") {
          ...
          lastNode =
            stepAry[i - 1] === undefined ? parent : stepAry[i - 1].name;
          ...
          endNode =
            stepAry[i + 1] === undefined ? endNode : stepAry[i + 1].name;
...
*/
  ...
  const getFirstNode = function (stepAry, parent) {
    ...
    if (stepAry.length === 0) {
      ...
      _.head(stepAry).type === "loop" ||
      _.head(stepAry).type === "branch"
    ) {
      let keysAry = [];
      ...
      _.head(stepAry).type === "normal" ||
      _.head(stepAry).type === "break"
    ) {
...
*/
  ...
  const getLastNode = function (stepAry, parent) {
    ...
    if (stepAry.length === 0) {
      ...
      _.last(stepAry).type === "loop" ||
      _.last(stepAry).type === "branch"
    ) {
      let ary = [];
      ...
      _.last(stepAry).type === "normal" ||
      _.last(stepAry).type === "break"
    ) {
    ...
    ary.forEach((el) => {
      if (el.type === "normal" || el.type === "break") {
        ...
        el.nextNode.forEach((el) => {
          if (
            el === undefined ||
            flatStructure.find((item) => item.name === el) === undefined
          ) {
            // skip
          } else {
            next.push(
              flatStructure.find((item) => item.name === el).description
            );
          }
        });
...
*/
  ...
  const nodeFind = function (obj, key) {
    let res = importJSON.workflows[0].steps.find((s) => s.name === obj.name)[
      key
    ];
    return res;
  };
  ...
  const structure = groupBuilder(
    importJSON.steps_structure,
    strReplace(importJSON.title)
  );
...
run(
  JSON.parse(
    fs.readFileSync(
      `/Users/bill.keiffer/Desktop/Git/TrayCode/EraserExport/IconLookup/Workflows/project_[PROD]-Bynder-Workfront.json`
    )
  )
);
